...
void setUnitSphere( Mesh & o_mesh, int nX=20, int nY=20) {
    ...
    nY = nY + 1;
    int nbVertices = (int)o_mesh.vertices.size();
...
void setTesselatedSquare(Mesh & o_mesh, int nX=200, int nY=200) {
    ...
    for(int i = 0; i < nX_square; i++) {
        ...
        for(int j = 0; j < nY_square; j++) {
            ...
            o_mesh.vertices.push_back(Vec3(i,j,z));
    ...
    // Calcul des normales à chaque face
    ...
    for(int i = 0; i < (int)o_mesh.triangles.size(); i++) {
        ...
        for(int j = 0; j < 3; j++) {
            ...
            o_mesh.normals[tri.v[j]] += faceNormal;
...
void drawTriangleMesh( Mesh const & i_mesh ) {
    ...
    for(int i = 0; i < i_mesh.vertices.size(); i++) {
        ...
        glVertex3f(i_mesh.vertices[i][0], i_mesh.vertices[i][1], i_mesh.vertices[i][2]);
...
void key (unsigned char keyPressed, int x, int y) {
    ...
    case '+': // Incrémenter nX et nY
        nX_sphere++;
        nY_sphere++;
        setUnitSphere(unit_sphere,nX_sphere,nY_sphere);
        break;

    case '-': // Décrémenter nX et nY
        nX_sphere--;
        nY_sphere--;
        setUnitSphere(unit_sphere,nX_sphere,nY_sphere);
        break;
